...
int createSemaphore(int32 ownerEnvID, char* semaphoreName, uint32 initialValue)
{
	...
	struct Semaphore *semaphoreObject;
	int semaphoreId = allocate_semaphore_object(&semaphoreObject);
...
void signalSemaphore(int ownerEnvID, char* semaphoreName)
{
	...
	if(semaphores[semaphoreId].value <= 0)
	{
		dequeuedEnv = dequeue(&semaphores[semaphoreId].env_queue);
		sched_insert_ready(dequeuedEnv);
		dequeuedEnv->env_status = ENV_READY;
	}
}
