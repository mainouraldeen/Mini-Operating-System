...
{
////////////////////// Placement ////////////////////////////
	int workingSetSize = env_page_ws_get_size(curenv);
	if(workingSetSize < curenv->page_WS_max_size){
		uint32 allPagePerms=pt_get_page_permissions(curenv,fault_va);
		if(allPagePerms & PERM_BUFFERED){//check if buffered
		...
		else{// lw el page msh buffered hgbha mn el page file
			struct Frame_Info* ptr_frame_info;
			int ret = allocate_frame(&ptr_frame_info) ;
			map_frame(curenv->env_page_directory, ptr_frame_info, (void*)fault_va, PERM_USER|PERM_WRITEABLE|PERM_PRESENT);

			int succ=pf_read_env_page(curenv,(void*)fault_va);
			//successfully read if 0 else not in page file check if stack page..if so therefore its new
		...
		//update working set
		int indx =curenv->page_last_WS_index;
		for(int i=indx;  ;i++){
			i=i% (curenv->page_WS_max_size);
				if(curenv->ptr_pageWorkingSet[i].empty ==1){//lw empty ha7ot feha
					indx=i; break;
				}
		}
		fault_va=ROUNDDOWN(fault_va,PAGE_SIZE);
		env_page_ws_set_entry(curenv,indx,fault_va);

		if(curenv->page_last_WS_index < curenv->page_WS_max_size-1){
			curenv->page_last_WS_index++;
		}
		else{
			curenv->page_last_WS_index=(indx+1) % (curenv->page_WS_max_size);
		}
...
////////////////////// Replacement////////////////////////////
	...
	{
	int try1=0, try2=0,victimVA,allPagePerms;
	struct Frame_Info* ptr_frame_info;
	int oneCycle=curenv->page_last_WS_index;
	//3)) modified clock algo
if(isPageReplacmentAlgorithmModifiedCLOCK()){
if(try1==0&&try2==0){
	while(1==1){//try1
		victimVA=env_page_ws_get_virtual_address(curenv,curenv->page_last_WS_index);
		allPagePerms=pt_get_page_permissions(curenv,victimVA);

		if((allPagePerms & PERM_USED)==0 &&( allPagePerms & PERM_MODIFIED)==0){
			curenv->page_last_WS_index++;
		...
		if(curenv->page_last_WS_index==oneCycle){// 1st try 1 completed
...
if(try1==0){
	...
	while(1==1){//try2
		...
		if((allPagePerms & PERM_USED)==0) {
			curenv->page_last_WS_index++;
			curenv->page_last_WS_index=curenv->page_last_WS_index%curenv->page_WS_max_size;
			try2=1;
			break;
		}
		pt_set_page_permissions(curenv,victimVA,0,PERM_USED);
...
if(try2faild==1){
	while(1==1){//try2 again
		...
		if(curenv->page_last_WS_index==oneCycle){// 2nd try 2 completed
			break;
		}
		}
}
}
	//4)) prepare victim frame info
	uint32* ptr_page_table=NULL;
	struct Frame_Info *ptr_victim_frame = get_frame_info(curenv->env_page_directory,(void*)victimVA,&ptr_page_table);

	ptr_victim_frame->isBuffered=1;
	ptr_victim_frame->environment=curenv;
	ptr_victim_frame->va=victimVA;

	pt_set_page_permissions(ptr_victim_frame->environment, victimVA, PERM_BUFFERED,PERM_PRESENT);

	//5)) if else b3den placement
	allPagePerms=pt_get_page_permissions(ptr_victim_frame->environment,victimVA);
	if((allPagePerms&PERM_MODIFIED)==0){
	...
	else
	{
		...
		//check count of modified list
		uint32 maxLen = getModifiedBufferLength();
		uint32 currentLen = LIST_SIZE(&modified_frame_list);
	...
	//6)) placement
	allPagePerms=pt_get_page_permissions(curenv,fault_va);
	if((allPagePerms & PERM_BUFFERED)>0){//check if buffered
		uint32* ptr_page_table=NULL;
		struct Frame_Info *ptr_frame_info = get_frame_info(curenv->env_page_directory,(void*)fault_va,&ptr_page_table);
		pt_set_page_permissions(curenv,fault_va,PERM_PRESENT,PERM_BUFFERED);
		ptr_frame_info->isBuffered=0;
	...
	if(curenv->page_last_WS_index==0){
	   env_page_ws_set_entry(curenv,curenv->page_WS_max_size-1,fault_va);
	}
	else{
		env_page_ws_set_entry(curenv,curenv->page_last_WS_index-1,fault_va);
	}
