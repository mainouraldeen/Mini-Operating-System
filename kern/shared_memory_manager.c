...
int createSharedObject(int32 ownerID, char* shareName, uint32 size, uint8 isWritable, void* virtual_address)
{
	...
	sharedObjectId = allocate_share_object(&sharedObject);
	...
	int requiredPages = size/PAGE_SIZE;
	if(size % PAGE_SIZE)
		requiredPages++;
	...
	for(int i=0; i<requiredPages; i++)
	{
		...
		int mappingFrameRet;
		//if(isWritable)
			mappingFrameRet = map_frame(curenv->env_page_directory,ptr_frame_info,(void*)virtual_address,PERM_PRESENT|PERM_WRITEABLE|PERM_USER);
		//else
		//	mappingFrameRet = map_frame(curenv->env_page_directory,ptr_frame_info,(void*)virtual_address,PERM_PRESENT|~PERM_WRITEABLE|PERM_USER);

		add_frame_to_storage(shares[sharedObjectId].framesStorage, ptr_frame_info, i);
	...
	shares[sharedObjectId].ownerID = ownerID;
	shares[sharedObjectId].size = size;
	strcpy(shares[sharedObjectId].name , shareName);
	shares[sharedObjectId].isWritable = isWritable;
	shares[sharedObjectId].references =1;
...
int getSharedObject(int32 ownerID, char* shareName, void* virtual_address)
{
	...
	int numOfFrames = shares[sharedObjId].size / PAGE_SIZE;
	if(shares[sharedObjId].size % PAGE_SIZE)
			numOfFrames++;
	for(int i=0; i<numOfFrames; i++)
	{
		struct Frame_Info* ptr_frame_info = NULL;
		ptr_frame_info = get_frame_from_storage(shares[sharedObjId].framesStorage,i);

		if(shares[sharedObjId].isWritable == 0)
			map_frame(myenv->env_page_directory, ptr_frame_info, virtual_address, /*~PERM_WRITEABLE|*/PERM_USER);
		else
			map_frame(myenv->env_page_directory, ptr_frame_info, virtual_address, PERM_WRITEABLE|PERM_USER);
