...
void * create_page_table(uint32 *ptr_page_directory, const uint32 virtual_address)
{
	...
	uint32 *returnedAddressKmalloc;
	returnedAddressKmalloc = (uint32*)kmalloc(PAGE_SIZE);
	uint32 physAddress = kheap_physical_address((unsigned int)returnedAddressKmalloc);
												//bt7ot gowa el page directory el page table bta3y
	ptr_page_directory[PDX(virtual_address)] = CONSTRUCT_ENTRY(physAddress , PERM_PRESENT | PERM_USER | PERM_WRITEABLE);
	memset(returnedAddressKmalloc , 0 , PAGE_SIZE);
	tlbflush();
	return (void*)returnedAddressKmalloc;
}
...
void allocateMem(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	//how e7na ezay msh han3ml map hy3raf mnean al page da taba3 al frame al folane
	//wa han3ml map fean aslan
	//cprintf("size in allocateMem fn: %d\n", size);
	uint32 startAddress=virtual_address;

	uint32 numberOfFrames=size;

	for(int i=0; i<numberOfFrames; i++)
	{
		uint32* ptr_page_table=NULL;

		//writing in page file
		pf_add_empty_env_page(e, startAddress, 0);
...
void __freeMem_with_buffering(struct Env* e, uint32 virtual_address, uint32 size)
{
	...
	uint32 start_address= virtual_address;
	uint32 numberOfFrames=size;
	//cprintf("address to be freed %x size = %d\n",virtual_address,size);

	//freeing buffered pages
	start_address= virtual_address;
	for(int i = 0; i < numberOfFrames; i++)
	{
		uint32 perm = pt_get_page_permissions(e, start_address);
		if ((perm & PERM_BUFFERED) > 0)//buffered
		{
			uint32 *ptr_page_table = NULL;
			struct Frame_Info *ptr_frame_info = get_frame_info(e->env_page_directory,(void*)start_address,&ptr_page_table);

			//remove it from modified list
			if ((perm & PERM_MODIFIED) > 0)
			{
			...
			//in both ML and FFL add the frame at the head of the list cause i don't need to hold it back anymore
			//LIST_INSERT_HEAD(&free_frame_list,ptr_frame_info);
			free_frame(ptr_frame_info);
			ptr_frame_info->isBuffered = 0;
			ptr_frame_info->environment=NULL;
			//Set its entry in the page table to NULL to indicate that this page is no longer exists
			// not present and not buffered
			pt_set_page_permissions(e,start_address, 0, PERM_PRESENT|PERM_BUFFERED|PERM_USER|PERM_MODIFIED);
		}
		start_address+=PAGE_SIZE;
	}

	//freeing pages from the working set
	start_address= virtual_address;
	for(int i = 0; i < numberOfFrames; i++)
	{
		env_page_ws_invalidate(e, start_address);
		start_address+=PAGE_SIZE;
	}

	//deleting empty page table from environment
	start_address= virtual_address;
	for(int i = 0; i < numberOfFrames; i++)
	{
		unmap_frame(e->env_page_directory, (void*)start_address);
		start_address+=PAGE_SIZE;
	}

	//deleting empty pageTables
	start_address= virtual_address;
	for(int i = 0; i < numberOfFrames; i++)
	{
		uint32 * page_table = NULL;
		get_page_table(e->env_page_directory, (void*)start_address, &page_table);
		if(page_table != NULL)//if page table exists
		{
			int isEmptyTable = 1;
			//checking if all page table entries are empty
			for(int i = 0; i < 1024; i++)
			{
				if((page_table[i] &PERM_PRESENT )!= 0)//page table is not empty
				{
					isEmptyTable = 0;
					break;
				}
			...
			if(isEmptyTable)
			{
				//free page table from heap

				// get the physical address and Frame_Info of the page table
				uint32 table_pa=(e->env_page_directory[PDX(start_address)]>>12)*PAGE_SIZE;
				...
				uint32 dir_index = PDX(start_address);
				e->env_page_directory[dir_index] = 0;

				//free page table from directory
				pd_clear_page_dir_entry(e,start_address);
			}
		}
		start_address+=PAGE_SIZE;
	}

	//freeing pages from the page file
	start_address= virtual_address;
	for(int i = 0; i < numberOfFrames; i++)
	{
		pf_remove_env_page(e, start_address);
		start_address+=PAGE_SIZE;
	}
}
